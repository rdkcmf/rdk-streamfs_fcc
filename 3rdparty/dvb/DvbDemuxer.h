...
#pragma once
#include "DvbCbHandler.h"
#include "DvbStreamListener.h"

namespace dvb {
class DvbDemuxer : public Demuxer {
public:
    explicit DvbDemuxer(uint32_t id) : Demuxer(id) {
        mCb = std::make_shared<DvbCbHandler>();
    }
...
private:
    std::shared_ptr<DvbCbHandler> mCb;
    DvbStreamListener mDvbSListener;
};
