...
class JSON {
    union BackingData {
        ...
        BackingData(unsigned long l) : UInt(l) {}
...
public:
    ...
    template<typename T>
    JSON(T i, typename enable_if<is_unsigned<T>::value && !is_same<T, bool>::value>::type * = 0) : Internal((unsigned long) i),
                                                                                                   Type(Class::UIntegral) {}

    template<typename T>
    JSON(T i, typename enable_if<is_integral<T>::value && !is_unsigned<T>::value && !is_same<T, bool>::value>::type * = 0) : Internal((long) i),
                                                                                                   Type(Class::Integral) {}
    ...
    long ToUInt(bool &ok) const {
        ok = (Type == Class::UIntegral);
        return ok ? Internal.UInt : 0;
    }
    ...
    string dump(int depth = 1, string tab = "  ") const {
        ...
        switch (Type) {
            ...
            case Class::UIntegral:
                return std::to_string(Internal.UInt);
             case Class::Boolean:
...
JSON parse_string(const string &str, size_t &offset) {
    JSON String;
    string val;
    for (char c = str[++offset]; c != '\"' && c != '\0'; c = str[++offset]) {
        if (c == '\\') {
            switch (str[++offset]) {
                case '\"':
                    val += '\"';
